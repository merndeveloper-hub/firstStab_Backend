...
const removeServiceSubCategory = async (req, res) => {
  try {
    await schema.validateAsync(req.params);
    const { id } = req.params;
    const findCategory = await findOne("proCategory", { _id: id });
    if (!findCategory || findCategory.length == 0) {
      return res.status(400).send({ status: 400, message: "Service Sub Category Not found" });
    }
...
export default removeServiceSubCategory;
