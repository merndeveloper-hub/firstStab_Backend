...
const cancelledBookedService = async (req, res) => {
  try {
    ...
    const cancelbooking = await updateDocument(
      "serviceDetail",
      { _id: id },
      { status: "Cancelled By Professional", ...req.body }
    );

    const cancelProBooking = await updateDocument(
      "proBookingService",
      {
        bookServiceId,
    ...
    const cancelUserBooking = await updateDocument(
      "userBookServ",
      {
        _id: bookServiceId,
        status: "Accepted" || "Pending",
...
export default cancelledBookedService;
